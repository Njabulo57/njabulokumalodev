...
function initializeTracking() {
  ...
  // Check if user has been to your site before
  //var hasVisited = localStorage.getItem('hasVisited');
  

  // Function to determine if referrer is from a different domain
  function isDifferentDomain(referrer) {
      if (!referrer) {
          return true;
      }
  ...
  // Execute the function if user is new or coming from a different domain
  if (isDifferentDomain(referrer)) {
      executeFunction();
  }else{
    checkScreen(bigScreens);
    //consentBanner.style.display = 'none';
  ...
  // Store a flag in local storage to mark the user as visited
